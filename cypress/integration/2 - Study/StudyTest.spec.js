...
describe('Study Test', () => {
	...
	it('Edit Study Basic information', () => {
		...
		cy.get('textarea[id^=basic_information_related_publications_]').each(($el, index) => {
	...
	it('Edit Data collection', () => {
		...
		cy.get('.add-another-collection-widget[data-list-selector="#measures-fields-list"]').click().click();
		cy.get('input[id^=measure_measures_]').each(($el, index) => {
			cy.wrap($el).clear().type(study['measure_measures_' + index]);
			if (index === 1) {
				cy.wrap($el).siblings('.remove-another-collection-widget').click();
			}
		});
		cy.get('.add-another-collection-widget[data-list-selector="#apparatus-fields-list"]').click().click();
		cy.get('input[id^=measure_apparatus_]').each(($el, index) => {
			cy.wrap($el).clear().type(study['measure_apparatus_' + index]);
			if (index === 1) {
				cy.wrap($el).siblings('.remove-another-collection-widget').click();
			}
		});
