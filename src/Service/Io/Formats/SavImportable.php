...
readonly class SavImportable
{
    ...
    public function savToArray(Dataset $dataset): array
    {
        ...
        if ($sav && array_key_exists('variables', $sav)) {
            $count = 1;
            foreach ($sav['variables'] as $var) {
                $cv['id'] = $count++;
                $cv['name'] = array_key_exists('name', $var) ? $var['name'] : '';
                $cv['label'] = array_key_exists('label', $var) ? $var['label'] : '';
                $cv['itemText'] = null;
                $cv['values'] = null;
                if (array_key_exists('values', $var) && is_iterable($var['values'])) {
        ...
        if ($sav && array_key_exists('dataMatrix', $sav)) {
            $data['records'] = $sav['dataMatrix'];
        }
        $this->logger->debug('SavImportable::savToArray:Leave with result size: '.count($data ?: []));
    ...
    private function _createMissings(?string $type, ?string $val1, ?string $val2, ?string $val3): ?array
    {
        ...
        $this->logger->debug('SavImportable::_createMissings:Enter with result size: '.count($missings ?: []));
    ...
    {
        $this->logger->debug('SavImportable::_createValueLabelArray:Enter with array size: '.count($val ?: []));

        return [
            'name' => array_key_exists('value', $val) ? $val['value'] : '',
            'label' => array_key_exists('label', $val) ? $val['label'] : '',
        ];
    }
}
